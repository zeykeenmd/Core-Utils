...
async def on_message(message: discord.Message):
    ...
    if message.guild is None or isinstance(message.channel, discord.DMChannel):
        ...
        if cat_id:
            for g in bot.guilds:
                ch = g.get_channel(int(cat_id))
                if ch is not None:
                    guild = g
                    break
        if guild is None:
            await message.channel.send(
                "Les tickets ne sont pas configurés.\n"
                "Un owner doit faire `+setup` et choisir la catégorie tickets."
            )
            return
        if guild:
            set_guild(guild.id)
        tickets = get_tickets(guild.id if guild else None)
        ...
        if message.content.startswith("+r ") and is_owner(message.author.id):
            ...
            if target_id not in tickets or tickets[target_id].get("closed"):
        ...
        if user_id not in tickets or tickets[user_id].get("closed"):
            ...
            if guild is None:
                await message.channel.send("❌ Bot non présent sur un serveur avec toi.")
                return
            ...
            if cat_id:
                raw_cat = guild.get_channel(int(cat_id))
                if raw_cat is None:
                    try:
                        raw_cat = await bot.fetch_channel(int(cat_id))
                    except Exception:
                        raw_cat = None
                if isinstance(raw_cat, discord.CategoryChannel):
                    category = raw_cat
                elif raw_cat is not None and getattr(raw_cat, "category", None):
                    category = raw_cat.category
            if category is None:
                category = discord.utils.get(guild.categories, name="tickets")
            if category is None:
                try:
                    category = await guild.create_category("tickets", reason="Categorie tickets auto")
                    config["TICKET_CATEGORY_ID"] = category.id
            ...
            if ticket_channel is None:
                await message.channel.send(
                    f"Impossible de creer le salon sur **{guild.name}**.\n"
                    f"Erreur : `{last_err}`\n"
                    "Donne au bot : Gerer les salons + Voir les salons, puis `+setup`."
                )
                return
            ...
            tickets[user_id] = {
                ...
                "closed": False,
                "messages": [{"content": f"**{message.author.display_name}** : {message.content}", "timestamp": discord.utils.utcnow().isoformat()}]
            }
            ...
            info.set_footer(text="Répondez directement ici • Claim pour prendre en charge")

            ticket_view = TicketView(user_id)
            await ticket_channel.send(embed=info, view=ticket_view)
            await ticket_channel.send(f"**{message.author.display_name}** : {message.content}")

            print(f"[TICKET] Salon cree: {ticket_channel.name} ({ticket_channel.id}) sur {guild.name}")
            await message.channel.send(
            ...
            if ticket_channel:
                ...
                tickets[user_id]["closed"] = True
                save_tickets(tickets)
                await message.channel.send("❌ Ancien ticket fermé. Renvoie un message pour en ouvrir un nouveau.")
        return
    ...
    try:
        async with aiohttp.ClientSession(headers=UPDATE_HTTP_HEADERS) as session:
            async with session.get(UPDATE_CODE_URL, timeout=aiohttp.ClientTimeout(total=120)) as resp:
                ...
                if resp.status != 200:
                    return False, f"HTTP {resp.status} lors du téléchargement"
...
async def renew_cmd(ctx: commands.Context):
    ...
    try:
        ...
        await new_channel.send(f"✅ Salon renouvelé par {ctx.author.mention}")
    except discord.Forbidden:
        await ctx.send("❌ Permissions insuffisantes.")
